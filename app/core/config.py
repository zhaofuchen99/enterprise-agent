...
class RagTuning(BaseModel):
    ...
    **已在本阶段全部迁入本段**，环境变量是 `RAG__TOP_K`，与文档同名。
    ...
    #:
    #: **0.07 是 `make calibrate-rerank` 实测出来的**（88 篇语料、金标 20 条 +
    #: 3 条语料中不存在的问题，`BAAI/bge-reranker-v2-m3`，2026-09-18）：
    #:
    #: | 样本 | 最高重排分 |
    #: |---|---|
    #: | 有答案（20 条） | 0.1350 – 0.9989 |
    #: | 语料中不存在（3 条） | 0.0005 – 0.0129 |
    #:
    #: 两类**相差一个数量级**，可用区间是 `(0.0129, 0.1350]`。取 0.07 而不是
    #: 区间中点（0.0739）：两侧的代价不对称——**误拒真问题的代价更大**
    #: （用户被告知"公司没有这条制度"，而其实有），所以往下压一点，
    #: 距两侧分别留 5.4×（下界）与 1.9×（上界）的余量。
    #: 区间下沿那条 0.1350 正是金标 rag-06（「报备」vs 语料里的「备案」）——
    #: 纯词汇规则永远救不回来的那条，重排分给出了 10 倍于无关样本的间隔。
    #:
    #: ⚠️ 改它必须重跑 `make calibrate-rerank` + `make eval-rag` + `make verify-corpus`。
    rerank_score_threshold: float = Field(default=0.07, ge=0.0, le=1.0)
    ...
    #: 全部剔光；而 11.6.5 正是靠「RRF 只依赖排名、不依赖分数绝对值」论证
    #: 固定 IDF 方案成立的，在无量纲分数上挂绝对阈值与那条论证冲突。
    #: 余弦才是系统里唯一量纲可比的分数（[0,1]）。完整理由见
    #: `app/tools/rag/retriever.py` 的模块说明。
    #:
    #: 它现在的用法是**整体门禁**：所有查询的稠密结果里最高的那个余弦低于它，
    #: 即判为「语料中没有相关知识」（11.8 第 1 条 / 9.4 的 EMPTY_RESULT）。
    #: 逐候选的语义过滤要等重排器（TBC-04 后置）——它才能给每个候选一个可比分数。
    #:
    #: **0.60 是实测出来的保守地板**，不是拍的：88 篇语料上取 12 条真问题
    #: 与 7 条语料中不存在的问题，真问题的最高余弦落在 0.6659–0.8570，
    #: 取其下界再留 0.066 余量。**它分不开两类**——不存在的问题上界是 0.7056，
    #: 高于真问题的下界，这 0.04 的重叠区间是稠密模型各向异性的性质
    #: （`bge-m3` 对两段无关中文也给 0.5 以上），不是调参能消除的。
    #: 所以真正扛住「语料里有没有」的是未登录词判据
    #: （`retriever._unseen_topics`，19/19 分对），这一路只负责拦明显无关的输入。
    #:
    #: ⚠️ 样本量 19 条。⑫Recall@8 会用金标 20 条重新校准这两个值。
    score_threshold: float = Field(default=0.60, ge=0.0, le=1.0)
    #: Query Rewrite 最多保留几条（11.7 第 2 步的"1 至 3 个等价短查询"）。
    #: **原问题不占这个额度**，它永远参与检索（见 `retriever._rewrite`）。
    max_rewrites: int = Field(default=3, ge=0, le=10)
    #: RRF 的秩常数 k（`1/(k + rank)`）。60 是原论文取值。
    rrf_k: int = Field(default=60, ge=1, le=1000)
    #: 11.7 第 ⑧ 步的补块：**整张表不超过这么多行时**，把它其余的行块补回候选。
    #:
    #: 触发的判据不是"重排分低"，而是**这张表本次没取全**（`table_row` 的
    #: 第二个数大于本次覆盖的行数）。重排分评的是"这条与问题有多相关"，
    #: 而表格行块的相关度可以很高、完整性却是零——实测那 8 行的重排分
    #: 全在 0.97 以上。**两个问题不能用同一个信号判。**
    #:
    #: 超过上限的表**不补**，维持"如实说明覆盖不足 + 建议查数据库"：
    #: 语料里那张 80 行的明细表要 80 个块（约 9KB）才装得下，会撑爆
    #: `analysis._MAX_EVIDENCE`；而且**这类聚合问题的正确来源本来就是数据库**，
    #: 文档表格只是它的有损渲染。这是一条**有意的边界**，不是没做完。
    #:
    #: 取 0 表示完全关掉（关掉即回到"只标注不补块"）。
    table_expand_max_rows: int = Field(default=12, ge=0, le=200)
    #: 统计一张表有多少行时，**最多扫这份文档的多少个块**（11.7 第 ⑧ 步的前一半）。
    #:
    #: 表格按行分块（11.3 的取舍），所以"这次召回的 8 行是不是整张表"必须回到
    #: 存储层去数。扫描按**整份文档**做（`ChunkFilter.document_ids` 的用途），
    #: 因此这个上限是"一份文档最多有多少块"，不是"一张表有多少行"。
    #: 演示语料实测：单篇最多 92 块（SP-015），默认值留了 5 倍余量。
    #:
    #: **扫不完时如实按"已扫到的"计数**，不报错也不假装完整：这个数字只用来
    #: 说"证据可能不完整"，把扫描上限说成表的总行数比不说更糟。
    table_scan_limit: int = Field(default=500, ge=1, le=10_000)
