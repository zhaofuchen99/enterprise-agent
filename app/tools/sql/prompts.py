...
#: SQL 生成模板。
#:
#: **版本 1.0.1 的改动**：加了一条「聚合结果不要用 COALESCE/IFNULL 兜零」。
#: 依据是一次实测（2026-09-20）：问"2026年1月华南的净销售额"，
#: 数据库里根本没有那个月的数据，而模型写了 `COALESCE(SUM(...), 0)` →
#: 工具拿到的是"一行 0.00"而不是"没有匹配行" → `has_no_values` 判不出来
#: → 这一步被当成"查到了"，`reflect` 于是不补另一路，
#: 最终答案里那段"没查到"的说法也就没了依据。
...
- **聚合结果不要用 `COALESCE` / `IFNULL` 兜零**：聚合函数在没有匹配行时返回 `NULL`，
  而 `NULL` 正是系统判断「按当前条件查不到数据」的依据（会据此去补另一路检索）。
  兜成 0 之后，**「没有数据」与「数据是 0」就再也分不开了**——
  那会让一个查不到的问题看起来像查到了。NULL 直接返回即可，
  下游会把它如实呈现为"没查到"；
