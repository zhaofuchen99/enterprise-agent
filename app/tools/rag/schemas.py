...
class RetrievedChunk(BaseModel):
    ...
    #: 融合后的名次，1 起
    rank: int
    #: 表格行块在**整张表**里的位置：`(第几行, 共几行)`，1 起。非表格块为 `None`。
    #:
    #: **表格按行分块（11.3 的取舍），于是"这张表还有别的行"在检索结果里
    #: 完全看不出来**：召回 8 行与召回整张表长得一模一样。模型因此会拿这 8 行
    #: 去求和、当成区域合计——实测踩到过（华南 Q3：8 行求和 7,146.22 万，
    #: 真值 13,249.31 万，Reviewer 给了 100 分）。这个字段是那个缺口的补丁：
    #: 有了它，"证据只覆盖了 16 行里的 8 行"才是一句能被写出来的话。
    #:
    #: 行号按 `char_start` 排，因此**跨页表格也是连续的**——
    #: 11.3 的表头还原保证了同表各行的序列化形态一致，排序依据的是原文位置。
    table_row: tuple[int, int] | None = None
