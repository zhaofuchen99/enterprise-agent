...
84. **补块（11.7 第 ⑧ 步）的触发判据与它的副作用**。
    原文把缺口判据挂在**重排分**上，而**重排分评的不是这件事**：
    它评"这条与问题有多相关"，而表格行块的相关度可以很高、完整性却是零——
    实测那 8 行的重排分全在 **0.97 以上**，靠它永远判不出缺口。
    本实现改用 `table_row`：**这张表本次没取全就是缺口**。
    **「答得全不全」与「相不相关」是两个问题，不能用同一个信号判。**

    补块**有边界**（`RAG__TABLE_EXPAND_MAX_ROWS`，默认 12）：
    语料里那张 80 行的明细表要 80 个块（约 9KB）才装得下，会撑爆
    `analysis._MAX_EVIDENCE`；而且**这类聚合问题的正确来源本来就是数据库**，
    文档表格只是它的有损渲染。超边界的表维持"如实说明 + 建议查库"。
    补回来的块**不参与重排**（它们在第 ⑦ 步之后才进来，这正是 11.7
    把第 ⑧ 步排在第 ⑦ 步之后的原因），因此 `fusion_score` 与 `rerank_score`
    都是 `None`——**填 0 会被读成"评了、垫底"**（同约定 59）。

    ⚠️ **它引出了一处更大的缺陷，而且是修在 SQL 侧的**：原来的表里只进来一行，
    「SQL 侧 scope 为空就不比这一项」（约定 41 的有意放行）没出过事；
    补块把五行的表整张放进证据之后，**另外四个区域的行全被拿去对华东的库值**，
    报出四条相对差 9–16% 的假冲突（2026-09-20 实测）。
    根因是 `SELECT SUM(net_amount) WHERE region_name='华东'` 的**粒度只存在于
    WHERE 里**，证据上看不出来。修法是 `validator._extract_scope_filters`
    把它抽出来（只认**等值**：范围条件表达的是区间，塞进 `scope` 会造出一条
    "取值不同"的判定、把真冲突静默跳过——漏报比多报贵），
    `SqlToolResult.scope_filters` → 证据 `scope`。
    修完 demo-cross 从 **5 条回到 1 条**。两个模块共用的列名映射也收进了
    `schemas.SCOPE_COLUMNS`——原先靠注释对齐，漂移的症状是"某类冲突再也检不出来"。
    ⚠️ 这类取舍的方向是**宁可多报**：多报看得出来（读者会问"华南这行凭什么
    对华东的库值"），漏报看不出来。

85. **演示问题集扩到九条，并支持按账号跑**。新增的两类覆盖是本次补的：
    - **答案层数值断言从 1 条到 3 条**：`demo-sql-q2` 换期间、
      `demo-sql-annual` 跨四个季度——换期间/换粒度是刻意的，
      它防的是"固化了一个数、而那个数碰巧总能被抄对"。
    - **权限边界**（`demo-scope`，`account: analyst`）：受限账号问授权范围外的
      区域，一次跑同时钉住三条链路——SQL 被谓词拦成空 →「已按数据权限限定在
      华东」；空结果触发 `reflect` 补 RAG（验收③）；文档来源没有数据权限这一层，
      限制里必须声明未经过滤。**同一个问题换个账号，限制就多出三条**，
      这是别处看不到、一句话能讲清的东西。
    `demo.py` 因此支持**按用例指定账号**（默认 admin，token 按账号缓存）。
    ⚠️ `make demo-ask Q="..." [ACCOUNT=...]` 是配套的**固化前**入口：
    项目的纪律是"演示问题必须先跑一遍再固化"，而在此之前那要先手改 YAML
    再跑 `--only`——那让"先跑一遍"变成了"先猜一遍"（`demo-expand` 那条
    「2024年1月」就是这么踩出来的）。新用例的 `stable` 都是复跑验过的。
    ⚠️ 账号变量叫 `ACCOUNT` 不叫 `AS`：**`AS` 是 make 的内建变量**
    （汇编器命令，值就是 `as`），`$(if $(AS),...)` 恒为真，命令会变成 `--as as`。

86. **引用要合并，而合并之后行号不能丢**（`final._cite_all`）。原先逐条按
    `evidence_id` 渲染，同一章节同一页的 8 个表格行块渲染出**8 句一模一样的话**——
    读的人第一反应是"格式坏了"，而它其实是"这条结论引用了同一张表的 8 行"，
    一件正常且必须能被看出来事。
    **合并的条件是"读起来是同一个地方"**：分组键就是 `_cite` 渲染出的那句出处，
    另立一套键会与渲染逻辑各说各话。
    合并后要留**行号区间**（`第 1–3、5 行`）——这正是登记里"引用能不能表达
    表的一部分"的答案：**能，而且必须能**，否则"引用整张表"与"引用其中 8 行"
    会渲染成同一句话，而约定 78 那个缺口（8 行当整表合计）就住在它们的差别里。
    顺带修掉两处同类疤痕：**书名号里外是同一串**（`title` 已含末级章节，
    而 `section_path` 是完整路径），以及 SQL 侧 `result_slice` **0 基**
    直接打出来会印出「结果第 0 行」（人读从 1 起，与证据标题同口径）。
